...
<div class="container">
  <div class="row">
    ...
    <div class="jumbotron">
      <div class="container">
        <h2 class="display-3"><%=rs.getString("studyName")%> / <%=rs.getString("studyId")%></h2>
      </div>
    </div>
    <div class="col-md-7">
      <h4><%=rs.getString("studyCategory")%></h4>
      <br>
      <p>프로젝트 시작일 : <%=rs.getString("studyStartDate")%></p>
      <br>
      <p>프로젝트 종료일 : <%=rs.getString("studyEndDate")%></p>
      <br>
      <p>최대 인원 : <%=rs.getInt("maxParticipant")%>명</p>
      <br>
      <p>프로젝트 설명 : <%=rs.getString("studyInfo")%>
      <p>파일 다운 : <a href="./fileDown.jsp?file_name=<%=rs.getString("studyFileName")%>"><%=rs.getString("studyFileName")%></a></p>
    </div>
  </div>
  <%
      }
